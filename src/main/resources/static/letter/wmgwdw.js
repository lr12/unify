var rough = function () {
    ...
    var b = Math.tan, c = Math.pow, d = Math.cos, e = Math.sin, f = Math.PI, g = Math.sqrt, h = Math.max, j = Math.min,
    ...
    class l {
        ...
        compare(d) {
            ...
            var e = k, f = k, g = 0, l = 0, m = this.a, n = this.b, b = this.c;
    ...
    class o {
        ...
        parseData(a) {
            var b = this.tokenize(a), c = 0, d = b[c], e = "BOD";
            for (this.segments = []; !d.isType(this.EOD);) {
                ...
                if (c + f < b.length) {
                    for (var h, j = c; j < c + f; j++) if (h = b[j], h.isType(this.NUMBER)) g[g.length] = h.text; else return void console.error("Parameter type is not a number: " + e + "," + h.text);
                    var i;
                    if (this.PARAMS[e]) i = {
                        key: e,
                        data: g
